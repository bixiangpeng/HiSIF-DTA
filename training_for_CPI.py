...
def train(model, device, train_loader,optimizer,ppi_adj,ppi_features,proGraph_loader,loss_fn,epoch):
    '''
    :param model:  A model needed to train.
    :param device: Device for loading models and data.
    :param train_loader:  Dataloader used to batch the input data.
    ...
    :param proGraph_loader: An auxiliary dataloader for batching all proteins in dataset.
...
def test(model, device, loader,ppi_adj,ppi_features,proGraph_loader):
    ...
    :return: Ground truth, predicted values, and predicted labels
    '''
    model.eval()
...
def main(args):
    ...
    for fold in range(1,6): # Five-fold cross-training
        ...
        for epoch in range(args.epochs):
            ...
            G, P ,_= test(model, device, test_loader, ppi_adj,ppi_features,proGraph_loader)
