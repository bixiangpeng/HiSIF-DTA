...
def line2voc_arr(line,letters):
    arr = []
    regex = '(\[[^\[\]]{1,10}\])'
    line = replace_halogen(line) #Regex to replace Br and Cl with single letters
...
def pad_sequences(vectorized_seqs, seq_lengths, properties):
    ...
    return torch.LongTensor(seq_tensor).to('cuda:1'), torch.LongTensor(seq_lengths).to('cuda:1'), torch.DoubleTensor(target).to('cuda:1')
...
#     print(seq_tensor)
    ...
    # Return variables
    # DataParallel requires everything to be a Variable
    return torch.LongTensor(seq_tensor).to('cuda:1'), torch.LongTensor(seq_lengths).to('cuda:1')
...
def getDataSet(FoldPath):
    df = pd.read_csv(FoldPath)
    DataSet = [[row['0'],row['1'],row['2'] ]for index, row in df.iterrows()]
    return DataSet
