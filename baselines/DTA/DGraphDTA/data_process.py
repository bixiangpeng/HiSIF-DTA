...
def create_dataset_for_test(dataset):
    ...
    # test_fold = [j for i in test_fold for j in i]
    ...
    train_smile = list(df_train['compound_iso_smiles'])
    test_smile = list(df_test['compound_iso_smiles'])
    compound_iso_smiles = list(set(train_smile+test_smile))
    ...
    for pair_ind in range(len(rows)):
        ...
        valid_test_count += 1
    csv_file = f'data/{dataset}/test.csv'
    ...
    #     target_graph[proteins[key]] = g
    #
    #
    # with open(f'data/{dataset}/mol_data.pkl','wb') as file:
    #     pickle.dump(smile_graph,file)
    # with open(f'data/{dataset}/pro_data.pkl','wb') as file:
