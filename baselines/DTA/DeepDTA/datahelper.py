...
class DataSet(object):
  def __init__(self, setting_no, seqlen, smilen, need_shuffle = False):
  ...
  def read_sets(self, FLAGS): ### fpath should be the dataset folder /kiba/ or /davis/
    test_fold = json.load(open(f"./data/{FLAGS.dataset}/folds/test_fold_setting1.txt"))
    train_folds = json.load(open(f"./data/{FLAGS.dataset}/folds/train_fold_setting1.txt"))
    return test_fold, train_folds


  def parse_train_test_data(self, FLAGS, with_label=True):
    tr_ligands =  json.load(open(f"./data/{FLAGS.dataset}/ligands_can.txt"), object_pairs_hook=OrderedDict)
    tr_proteins = json.load(open(f"./data/{FLAGS.dataset}/proteins.txt"), object_pairs_hook=OrderedDict)
    tr_Y = pickle.load(open(f"./data/{FLAGS.dataset}/Y","rb"), encoding='latin1') ### TODO: read from raw
    te_ligands =  json.load(open(f"./data/{FLAGS.dataset}/ligands_can.txt"), object_pairs_hook=OrderedDict)
    te_proteins = json.load(open(f"./data/{FLAGS.dataset}/proteins.txt"), object_pairs_hook=OrderedDict)
    te_Y = pickle.load(open(f"./data/{FLAGS.dataset}/Y","rb"), encoding='latin1') ### TODO: read from raw
