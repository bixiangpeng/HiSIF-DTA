...
日期：2023年04月10日
"""
from models.HGCN_for_Ablation import *
from utils import *
import pandas as pd
import numpy as np
...
def test(model, device, loader,ppi_adj,ppi_features,pro_graph):
    ...
    with torch.no_grad():
        for data in loader:
            mol_data = data[0].to(device) # A batch data of drug graph.
            pro_data = data[1].to(device) # A batch data of protein graph.
...
def main(args):
    ...
    model_dict_ = {'LSM': LSM, 'HSM':HSM, 'HFS':HFS} # Three ablation variants.
    modeling = model_dict_[args.model]
    model_st = modeling.__name__

    path = f'results/{dataset}/ablation/{model_st}.model'
    device = torch.device(f'cuda:{args.device}' if torch.cuda.is_available() else "cpu")

    check_point = torch.load(path,map_location=device) # Loading the model parameters trained on the KIBA dataset.
    model = modeling()
    model.load_state_dict(check_point) # Loading pre-trained model parameters into the current model.

    model = model.to(device)
    df_test = pd.read_csv(f'data/{dataset}/test.csv') # Reading test set data.
    ...
    df_pre.to_csv(f'./results/{args.dataset}/ablation/predicted_value_of_{model_st}_on_{args.dataset} .csv') #Record the prediction results to a CSV file.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type = str, default = 'LSM', choices = ['LSM', 'HSM', 'HFS'])
    parser.add_argument('--batch', type = int, default = 512)
    parser.add_argument('--device', type = int, default = 1)
    parser.add_argument('--dataset', type = str, default = 'davis',choices=['davis'])
